...
parseISAW (JSON.Object o) | JSON.Null <- HMap.lookupDefault JSON.Null "is_part_of" o =
  return $ Just $ HMap.map parseValue o
parseISAW _ = return Nothing
...
streamISAW = mapM_ (\e -> entry e >> streamISAW) =<< C.await where
  entry (Left Z.ZipEntry{ Z.zipEntryName = n })
    | ".json" `BS.isSuffixOf` n =
      mapM_ C.yield =<< parseM parseISAW =<< whileRight C..| sinkParser JSON.json'
    | otherwise = return ()
